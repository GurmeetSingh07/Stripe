...
class stripecontroller {
  ...
  createOrder = async (req, res) => {
    try {
      ...
      if (
        ...
        // const docs = await productData.findOne({
        //   $and: [{ productName: productName }, { productPrice: productPrice }],
        // });
        // console.log(docs);
        ...
        if (true) {
          ...
          for (let key in orderdetail) {
            for (let internalKey in orderdetail[key]) {
              const quantity = orderdetail[key][internalKey];
              const result = await productData.findOne({
                productName: internalKey,
              });
              let total = quantity * result.productPrice;
              console.log("=>" + total);
              sum += total;
            }
          }
  ...
  productAdd = async (req, res) => {
    ...
    try {
      ...
      if (!productName || !productPrice) {
        return res
          .status(206)
          .json({ message: "Please Fill the Fields", success: false });
      }
  ...
  productView = async (req, res) => {
    try {
      ...
      console.log(productInfo);
      return res.status(200).json({
        message: `${productInfo.length} products found`,
        data: productInfo,
        success: true,
      });
    } catch (e) {
